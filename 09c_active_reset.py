"""
        ACTIVE RESET
This script is used to benchmark different types of q1_xy initialization including active reset protocols.
...
(q1_xy in |e>), do nothing (q1_xy in |g>) or measure again if the q1_xy state is undetermined (active_reset_two_thresholds).

Then, after q1_xy initialization, the IQ blobs for |g> and |e> are measured again and the readout fidelity is derived
similarly to what is done in IQ_blobs.py.

Prerequisites:
    - Having found the resonance frequency of the resonator coupled to the q1_xy under study (resonator_spectroscopy).
    - Having calibrated q1_xy pi pulse (x180) by running q1_xy, spectroscopy, rabi_chevron, power_rabi and updated the config.
...
# If I > threshold_e, then the q1_xy is assumed to be in |e> and a pi pulse is played to reset it.
# If I < threshold_g, then the q1_xy is assumed to be in |g>.
# else, the q1_xy state is not determined accurately enough, so we just measure again.
ge_threshold = 0
...
def qubit_initialization(method: str = "thermalization"):
    ...
    :return: the number of tries to reset the q1_xy.
    """
...
def active_reset_one_threshold(threshold_g: float, max_tries: int):
    ...
    If the q1_xy is in |e> (I>threshold), then play a pi pulse and measure again, else (q1_xy in |g>) return the number
    of pi-pulses needed to reset the q1_xy.
    ...
    :param max_tries: maximum number of iterations needed to reset the q1_xy before exiting the loop anyway.
    :return: the number of tries to reset the q1_xy.
    """
    ...
    with while_((I_reset > threshold_g) & (counter < max_tries)):
        # Measure the state of the rl1
        measure("readout", "rl1", None, dual_demod.full("rotated_cos", "rotated_sin", I_reset))
        align("rl1", "q1_xy")
        # Wait for the rl1 to deplete
        wait(depletion_time * u.ns, "q1_xy")
        # Play a conditional pi-pulse to actively reset the q1_xy
...
def active_reset_two_thresholds(threshold_g: float, threshold_e: float, max_tries: int):
    ...
    If I > threshold_e, then the q1_xy is assumed to be in |e> and a pi pulse is played to reset it.
    If I < threshold_g, then the q1_xy is assumed to be in |g> and the loop can be exited.
    else, the q1_xy state is not determined accurately enough, so we just repeat the process.
    The program waits for the rl1 to deplete before playing the conditional pi-pulse so that the calibrated
    pi-pulse parameters are still valid.

    :param threshold_g: threshold "inside" the |g> blob, below which the q1_xy is in |g> with great certainty.
...
def active_reset_fast(threshold_g: float):
    """
    Active reset protocol where the outcome of the measurement is compared to a pre-calibrated threshold (IQ_blobs.py).
    If the q1_xy is in |e> (I>threshold), then play a pi pulse, else (q1_xy in |g>) do nothing and proceed to the sequence.
    The program waits for the rl1 to deplete before playing the conditional pi-pulse so that the calibrated
    pi-pulse parameters are still valid.
...
with program() as active_reset_prog:
    ...
    with for_(n, 0, n < n_shot, n + 1):
        ...
        align()
        # Measure the state of the resonator after reset, q1_xy should be in |g>
        measure(
            "readout",
            "rl1",
            None,
        ...
        align("q1_xy", "rl1")
        # Measure the state of the rl1, q1_xy should be in |e>
        measure(
            "readout",
            "rl1",
            None,
        ...
        save(I_e, I_e_st)
        save(Q_e, Q_e_st)
        # Save only the count when the q1_xy was not directly measured in |g>
        with if_(count > 0):
            save(count, tries_st)
...
if simulation:
    ...
    # Fetch the 'I' & 'Q' points for the q1_xy in the ground and excited states
