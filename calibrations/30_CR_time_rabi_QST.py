...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    node.parameters.qubit_pairs = ["q16-17"]
    node.parameters.use_state_discrimination = True
    node.parameters.num_shots = 500
    node.parameters.max_wait_time_in_ns = 2000
    node.parameters.time_step_in_ns = 20

    node.parameters.wf_type = "square"
    node.parameters.cr_type = "direct"
    node.parameters.cr_drive_amp_scaling = 0.3
    node.parameters.cr_drive_phase = 0.0
    node.parameters.cr_cancel_amp_scaling = 0.0
    ...
    with program() as node.namespace["qua_program"]:
        ...
        for multiplexed_qubit_pairs in qubit_pairs.batch():
            ...
            with for_(n, 0, n < n_avg, n + 1):
                ...
                with for_(*from_array(t, pulse_cycle)):
